...
# Starting from folder 78 (skipping problematic folder 77)
# Keeping both DOCX and Markdown files
...
echo -e "${BLUE}=== Mexican Laws Conversion (Starting from Folder 78) ===${NC}"
echo -e "${BLUE}DOC → DOCX → Markdown (Keeping both files, skipping folder 77)${NC}\n"

# Count directories from 078 onwards
TOTAL=$(find . -maxdepth 1 -type d -name "[0-9]*" | grep -E "^./0(7[8-9]|[8-9][0-9]|[1-9][0-9][0-9])" | wc -l | tr -d ' ')
echo -e "${BLUE}Found $TOTAL law directories to process (starting from 078)${NC}"
echo -e "${YELLOW}⚠️  Skipping folder 077 (24MB DOC file caused LibreOffice to hang)${NC}\n"

# Process each directory starting from 078
counter=0
for dir in */; do
    ...
    if [ -n "$doc_file" ]; then
        ...
        if [ "$doc_size_mb" -gt 20 ]; then
            echo -e "  ${YELLOW}⚠️  Large DOC file detected: ${doc_size_mb}MB${NC}"
            echo -e "  ${YELLOW}⚠️  Skipping to avoid LibreOffice hang${NC}"
            ((SKIPPED++))
            cd ..
            echo ""
            continue
        ...
        echo -e "  ${BLUE}📁 Code: $law_code (${doc_size_mb}MB)${NC}"
        
        # Step 1: DOC to DOCX with timeout
        echo -e "  ${YELLOW}Step 1: DOC → DOCX${NC}"
        
        # Use timeout to prevent hanging
        if timeout 300s soffice --headless --convert-to docx "$doc_file" 2>/dev/null; then
...
# Final report
echo -e "${BLUE}=== Conversion Complete (From Folder 78) ===${NC}"
...
echo -e "${YELLOW}⚠️  Large files skipped: $SKIPPED${NC}"
...
echo -e "\n${GREEN}🎉 Conversion completed from folder 78!${NC}"
echo -e "${YELLOW}📁 Each folder contains: DOC + DOCX + Markdown files${NC}"
echo -e "${YELLOW}⚠️  Note: Folder 077 skipped due to 24MB file size${NC}"
